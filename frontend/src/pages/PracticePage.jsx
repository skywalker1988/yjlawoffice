...
import { Scale, ShieldAlert, Heart, Building2, Landmark, Receipt, Home, FileText, Mail, Handshake, Briefcase } from "lucide-react";
...
const PRACTICE_DEFAULTS = {
  ...
  intro: { description: "윤정 법률사무소는 다양한 법률 분야에서 축적된 경험과 전문성을 바탕으로 의뢰인에게 최적의 법률 솔루션을 제공합니다." },
  areas: { items: PRACTICES.map((p) => ({ title: p.title, subtitle: p.subtitle, desc: p.desc, details: p.details })) },
      ...
      <section className="section" style={{ background: "#fff" }}>
        <div className="container" style={{ maxWidth: 960 }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 stagger">
            {settings.areas.items.map((p, i) => {
              const Icon = PRACTICES[i]?.icon;
              return (
                <div
                  key={i}
