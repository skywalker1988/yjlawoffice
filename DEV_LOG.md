...
- **기간**: 2026-04-05 ~ 2026-04-11 (진행 중)
- **작업 범위**: 전체 코드 리뷰 → 보안 수정 → 클린코드 리팩토링 → 업무분야 페이지 추가 → 버그 수정
...
## 9차 작업: 건설·부동산 업무분야 페이지 추가 — 2026-04-11

### 9-1. 신규 페이지 2개 생성

**PracticeConstructionPage.jsx (444줄)**
- 건설 분쟁 전문 업무분야 상세 페이지
- 히어로 섹션: 건설 현장 배경 이미지 + 핵심 통계 (승소율, 누적 회수액 등)
- 의뢰인 고민 공감 섹션: 건설 분쟁에서 흔히 겪는 문제 상황 제시
- 전문 서비스 영역: 공사대금 청구, 하자보수, 공기 지연, 설계 변경 등
- 성공 사례 섹션: 주요 건설 분쟁 해결 실적
- CTA: 상담 신청 유도

**PracticeRealEstatePage.jsx (441줄)**
- 부동산 분쟁 전문 업무분야 상세 페이지
- 히어로 섹션: 부동산 배경 이미지 + 핵심 통계
- 부동산 거래/임대차/재개발·재건축/등기 분쟁 등 상세 서비스 영역
- 성공 사례 + 상담 신청 CTA

### 9-2. PracticePage 전면 리뉴얼

- 기존 10개 업무분야 카드 나열 방식 → 건설·부동산 전문 로펌 소개 + 상세 페이지 연결 구조로 변경
- 의뢰인 고민 공감 → 성공 사례 → 전문성 차별화 → 분야 선택 플로우
- 아이콘 변경: 기존 11개 (Scale, ShieldAlert, Heart 등) → 6개 (Building2, Home, ArrowRight 등)
- `useSiteSettingsPage` 훅 의존 제거

### 9-3. 라우팅 추가

- `App.jsx`: 2개 라우트 추가
  - `/practice/construction` → `PracticeConstructionPage`
  - `/practice/realestate` → `PracticeRealEstatePage`
- 두 페이지 모두 `ErrorBoundary` 래핑

### 9-4. 이미지 리소스 추가

- `frontend/public/construction-hero.jpg` (597KB)
- `frontend/public/construction-hero2.jpg` (745KB)
- `frontend/public/construction-hero3.jpg` (483KB)
- `frontend/public/construction-hero4.jpg` (203KB)
- `frontend/public/realestate-hero.jpg` (204KB)

### 9-5. CSS 추가

- `index.css`: `fadeIn` 키프레임 애니메이션 추가 (탭 전환 시 페이드인 효과)

### 9-6. 변경 파일 요약

| 파일 | 변경 |
|------|------|
| `PracticeConstructionPage.jsx` | 신규 (444줄) |
| `PracticeRealEstatePage.jsx` | 신규 (441줄) |
| `PracticePage.jsx` | 전면 리뉴얼 (238줄, +196/-165) |
| `App.jsx` | 라우트 2개 + import 2개 추가 |
| `index.css` | fadeIn 키프레임 추가 |
| `frontend/public/` | 히어로 이미지 5개 추가 |

---

## 10차 작업: 버그 수정 + 문서 정비 — 2026-04-11 (미커밋)

### 10-1. Vite 프록시 포트 수정

- `frontend/vite.config.js`: 프록시 대상 포트 `5001` → `5000`으로 복원
  - `/api/sb`, `/uploads`, `/data` 3개 프록시 모두 `localhost:5000`으로 통일
  - 백엔드 실제 포트(5000)와 불일치하던 문제 해결

### 10-2. CSRF 테스트 코드 수정

- `backend/tests/lib/csrf.test.js`: HMAC 서명 기반 CSRF로 변경된 것에 맞게 테스트 업데이트
  - `issueToken()` 헬퍼 함수 추가 — 미들웨어를 통해 유효한 CSRF 토큰 발급
  - "이미 csrf-token 쿠키가 있으면 새로 설정하지 않는다" 테스트: 하드코딩 `"existing-token-value"` → `issueToken()`으로 유효한 토큰 사용
  - "보호된 경로에 올바른 토큰을 보내면 통과한다" 테스트: 하드코딩 `"valid-csrf-token-12345"` → `issueToken()`으로 실제 서명된 토큰 사용
  - HMAC 서명 검증을 통과하지 못하던 기존 테스트가 정상 동작하도록 수정

### 10-3. 문서 업데이트

- `CLAUDE.md`: 프록시 설명 `"/api/sb → 5000 프록시"` → `"/api/sb·/uploads·/data → 5000 프록시"`로 정확한 프록시 경로 반영
- `README.md`: 동일하게 프록시 경로 3개(`/api/sb/*`, `/uploads/*`, `/data/*`) 명시
